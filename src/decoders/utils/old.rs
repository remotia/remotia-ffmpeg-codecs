...
mod old {
    pub fn decode_to_yuv(
        ...
        let decode_result = if let Some(error) = parse_packets(decode_context, parser_context, encoded_buffer, timestamp) {
            Err(error)
        } else {
            loop {
                match decode_context.receive_frame() {
                    Ok(avframe) => {
                        ...
                        write_avframe_to_yuv(avframe, y_buffer, cb_buffer, cr_buffer);
